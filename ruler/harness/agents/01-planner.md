...
- `vision.md` (필수, 유저가 작성한 최종 비전·범위 — **sub-goal 근거 인용의 출처**)
- `docs/PLAN.md` (필수, 상세 설계·아키텍처·마일스톤(M0~M3) — **sub-goal의 분해 단위·순서는 이 설계와 정합해야 함**)
- Researcher의 리서치 리포트 (선택)
- 이전 반복의 완료된 sub-goal 이력 (선택)

> sub-goal 근거 인용(Traceability)은 `vision.md` 기준을 유지하되, 분해·순서·기술 범위는 `docs/PLAN.md`의 마일스톤·아키텍처를 따른다.
> PLAN.md와 vision.md가 어긋나면 자의적으로 메우지 말고 유저에게 정합화를 요청한다.
