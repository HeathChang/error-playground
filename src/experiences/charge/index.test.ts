/**
 * charge Experience — 실제 마운트/언마운트 라이프사이클 (jsdom).
 *
 * game.test.ts가 ChargeGame 순수 상태 전이를 검증한다면, 여기서는 실제 배포 경로를 검증한다:
...
describe('charge Experience — mount/unmount 라이프사이클 (v2)', () => {
...
describe('charge Experience — 지원 환경(Canvas 2D) 렌더/루프 경로 (v2)', () => {
  ...
  it('should load and mount through the core deployment path (mount(el,{experience:"charge"}))', async () => {
    stubCanvas2d();
    captureRaf();
    const events: string[] = [];
    const container = mountedContainer();
