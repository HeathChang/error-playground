/**
 * zigzag Experience — 실제 마운트/언마운트 라이프사이클 (jsdom).
 *
 * game.test.ts가 ZigzagGame 순수 상태 전이를 검증한다면, 여기서는 실제 배포 경로를 검증한다:
...
/** "지원 환경(Canvas 2D)" 재현용 최소 컨텍스트 스텁 → canvasSupported()===true. */
function stubCanvas2d(): { fillRect: number; fillText: number; stroke: number } {
...
describe('zigzag Experience — mount/unmount 라이프사이클 (v2)', () => {
...
describe('zigzag Experience — 지원 환경(Canvas 2D) 렌더/루프 경로 (v2)', () => {
  ...
  it('should draw to the 2D context on mount, run the loop after input, and stop drawing after unmount', () => {
    ...
    expect(calls.fillRect + calls.fillText + calls.stroke).toBeGreaterThan(0);
    ...
    const before = calls.fillRect + calls.fillText + calls.stroke;
    raf.step(16);
    raf.step(200);
    expect(calls.fillRect + calls.fillText + calls.stroke).toBeGreaterThan(before);
    ...
    const afterUnmount = calls.fillRect + calls.fillText + calls.stroke;
    raf.step(216);
    expect(calls.fillRect + calls.fillText + calls.stroke).toBe(afterUnmount);
  });

  it('should load and mount through the core deployment path (mount(el,{experience:"zigzag"}))', async () => {
    stubCanvas2d();
    captureRaf();
    const events: string[] = [];
    const container = mountedContainer();
