...
| 경험(게임) | **러너·큐브·플래피·스태커·오빗·리듬·중력반전·타이밍스톱** + contract 검증용 `noop` | 다수 게임 무한 확장(장르 중복) | 공통 콘솔로 카트리지처럼 구동 (vision §4, 2026-07-08~07-17) |
...
게임은 공통 콘솔 **`gameMachine`** 에 꽂는 **카트리지(`Game`)** 다. 콘솔이 배관을 전담하므로 게임은 순수 로직만 작성한다 → 러너·큐브·플래피·스태커·오빗·리듬·중력반전·타이밍스톱이 동일 lifecycle을 공유한다.
...
`'noop'`(contract 검증) · `'runner'` · `'iframe'` · `'cube'` · `'flappy'` · `'stacker'` · `'orbit'` · `'rhythm'` · `'gravity'` · `'timing'`.
...
| **2** | Canvas 2D | ~10–20KB | **러너·플래피·스태커·오빗·리듬·중력반전·타이밍스톱** | 없음 | ✅ |
...
│       ├── runner|flappy|stacker|orbit|rhythm|gravity|timing/  # Canvas 카트리지 (game/render/index/*.test)
│       ├── cube/           # CSS-3D 카트리지
...
| **진행 — 신규 게임 추가** | 요청 "게임 추가" → 카트리지 1종씩 신설 (중력반전 2026-07-16, 타이밍스톱 2026-07-17) | ✅ (vision §4/§10 갱신) |
...
> 게임은 공통 콘솔 `gameMachine`에 꽂는 **카트리지**로, 러너·큐브·플래피·스태커·오빗·리듬·중력반전·타이밍스톱이 동일 lifecycle·입력·이벤트를 공유한다(§5.5). 통합은 `mount()` + 표준 Custom Element `<error-playground>`로 모든 프레임워크를 커버한다(§6.4).
